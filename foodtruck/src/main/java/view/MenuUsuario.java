...
public class MenuUsuario {
	...
	// Cadastro feito pelo novo Usuario
	...
	private void cadastrarUsuario(UsuarioVO usuarioVO) {
		if (usuarioVO.getTipoUsuario() == null) {
			do {
				usuarioVO.setTipoUsuario(TipoUsuarioVO.getTipoUsuarioVOPporValor(this.apresentaOpcoesTipoUsuario()));
	...
	private int apresentaOpcoesTipoUsuario() {
		UsuarioController usuarioController = new UsuarioController();
		ArrayList<TipoUsuarioVO> listaTipoUsuarioVO = usuarioController.cosultarTipoUsuario();
		System.out.println("\n ---- Tipo de Usuários ----");
		...
		while (opcao != OPCAO_MENU_CONSULTAR_USUARIO_VOLTAR) {
			...
			case OPCAO_MENU_CONSULTAR_TODOS_USUARIOS: {
				...
				System.out.printf("\n%3s  %-13s  %-20s  %-11s  %-25s  %-13s  %-24s  %-24s  %-10s  %-10s  ", "ID",
						"TIPO USUÁRIO", "NOME", "CPF", "E-MAIL", "TELEFONE", "DATA CADASTRO", "DATA EXPIRAÇÃO", "LOGIN",
						"SENHA");
			...
			case OPCAO_MENU_CONSULTAR_UM_USUARIO: {
				...
				if (usuarioVO.getIdUsuario() != 0) {
					...
					System.out.print("\n---------- RESULTADO DA CONSULTA ----------");
					System.out.printf("\n%3s  %-13s  %-20s  %-11s  %-25s  %-13s  %-24s  %-24s  %-10s  %-10s  ", "ID",
							"TIPO USUÁRIO", "NOME", "CPF", "E-MAIL", "TELEFONE", "DATA CADASTRO", "DATA EXPIRAÇÃO",
							"LOGIN", "SENHA");
					usuario.imprimir();
					System.out.println();
				} else {
					System.out.println("O campo código do usuário é obrigátorio");
				}
				break;
			}
			default: {
